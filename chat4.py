#== chat4.py ==
##-- A.B.Glazov -- 07/10/2020 --
##-- чат, подключаем передачу файлов
...
##--<4>-- списое партнеров (добавил время)
lst_partn = [("all","255.255.255.255",-1),("piter","192.168.1.200",0),("myself","127.0.0.1",-1)]
...
##--<4>-- функция приема файлов для потока сервера
def work_intftp():
    cur_dir = os.getcwd()
    tftpServer = minimumTFTP.Server( cur_dir )
    tftpServer.run()
    while True:
        pass
        sleep(0.001)
...
##--<4>-- кнопка отправки файла
def fnc_sendfile():
    #-- заблокировать широковещательную отправку
    if HOST_OUT == '255.255.255.255':
        MsgBox = messagebox.showwarning("Отказ","Файл можно посылать только конкретному пратнеру")       
        return
    
    #-- выбрать файл из диалогового окна
    file_name =  filedialog.askopenfilename(initialdir = ".",title = "Select file to send",
                    filetypes = (("all files","*.*"),("txt files","*.txt")))
    if len(file_name) < 1:  return
    delim_pos = file_name.rfind('/')
    #print( delim_pos)
...
ttk.Button( pnl_right, text = "отправить файл", width = btn_width,
    command = fnc_sendfile).grid(row = 7, column = 0, sticky=W, pady = 5, padx = 5)
...
##--<4>-- создать и запустить поток приема файлов
tr_intftp = threading.Thread( target = work_intftp)
tr_intftp.daemon = True
tr_intftp.start()
